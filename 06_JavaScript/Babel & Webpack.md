        ...
        optimization: {
          ...
          minimizer: [
            new TerserPlugin({
              ...
              // Minify 과정에서 코드가 변경되기에 디버깅이 어려워질 수 있음
              // 이를 해결하기 위해 source map을 생성하는 것이 좋음
              // 빌드된 코드를 원본 소스 코드에 매핑해주어, 브라우저의 개발자 도구에서 원본 코드를 보면 디버깅 가능케 해줌
              sourceMap: true,
              // 빌드 시간을 줄이기 위해 캐싱 기능 사용 가능
              // 이미 처리한 파일들을 캐시에 저장하고, 변하지 않은 파일은 다시 처리하지 않아 빌드 시간 단축
              cache: true,
              // 대규모 프로젝트의 경우, 빌드 시간을 단축하기 위해 병렬 처리 기능 활용 가능
              // 여러 CPU 코어를 이용하여 작업을 병렬로 처리 가능
              parallel: true, // 기본값은 os.cpus().length - 1
            })
          ]
        }
      }
